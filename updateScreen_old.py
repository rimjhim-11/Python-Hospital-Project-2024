...
def get_update_screen(root_window: object, local_data_list: object) -> object:
    ...
    for i in range(9):
        ...
        #print(f"Local data is: {local_data_list[i+1]}")
        print(update_.widget_var_list[i].get())
        print(f"widget details {update_.widget_var_list[i]}")
        print(f'The local_data_list[i+1] is {local_data_list[i+1]} and i is  {i}')

        update_.widget_var_list[i].set(local_data_list[i+1])
        #print(widget_var_list[i].get())
    update_entry_button = Button(update_screen, text="Update Entry",
                                 command=lambda: update_with_new_data(code, update_))
...
def update_with_new_data(code, update_):
    ...
    for row in range(page.max_row+1):
        ...
        if cell.value == code_temp:
            ...
            entry_found = True
            for i in range(9):
                #print(f"value of i in update screen is {i}")
